...
# filtered_keywords = re.sub(regex_filter, "|", example_keywords)
# keywords_split = [re.sub("\[*'*,*\s*\]*", " ", keyword_arr).split() for keyword_arr in filtered_keywords.split("|")
...
for i in range(7):
    ...
    mask_input =  example_keywords.replace(replace_word, model.tokenizer.mask_token)
