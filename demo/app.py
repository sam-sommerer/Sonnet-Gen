...
def rhyme_gen(sentence, add_radio):
    ...
    for i in range(7):
        ...
        replace_word = sentence.split(" Keywords")[countin_rhyming_lines[i]].split("'")[
            5
        ]
        ...
        if not found:
            ...
            if (
                not found
            ):  # if not found in masking outputs use random token to replace word, change 2 to randint
...
title = st.text_input(
    "Enter a title to generate the sonnet. Please also choose the type of sonnet from the sidebar",
    write_here,
)
...
with st.sidebar:
    ...
    st.write(
        "Zest does not require training on any poetic data. It consists of four components: content planning, rhyme pairing, polishing for aesthetics, and final decoding. The first three steps \
    ...
    add_radio = st.radio("Choose a type of sonnet", ("Petrarchan", "Shakespearean"))
...
if st.session_state["Generate Keywords"]:
    ...
    if st.button("Generate Rhyme Words"):
        st.session_state["Generate Rhyme Words"] = not st.session_state[
            "Generate Rhyme Words"
        ]

if st.session_state["Generate Keywords"] and st.session_state["Generate Rhyme Words"]:
    st.text_area(
        "The generate rhyme words for the given keywords",
        value=rhyme_gen(edited_keyword, add_radio),
    )
