    ...
    def load_data(self, path):
        ...
        for split in self.data:
            ...
            if split != "dev" or self.opt.data.devversion != "12":
                ...
                string_tuples = (
                    open("{}/{}".format(path, "dev1.txt"), "r").read().split("\n")
                )
                tuples = [x.split("\t") for x in string_tuples if x]
                string_tuples = (
                    open("{}/{}".format(path, "dev2.txt"), "r").read().split("\n")
                )
                tuples += [x.split("\t") for x in string_tuples if x]
    ...
    def make_tensors(
        ...
        for split in splits:
            ...
            if split == "train":
                self.data[split]["total"] = [
                    j
                    for i, j in enumerate(self.data[split]["total"])
                    if i not in set(discarded)
                ]
            self.masks[split]["total"] = [
                (len(i[0]), len(i[1]), len(i[2])) for i in sequences[split]
            ]
        ...
        self.max_e2 = max(
            [max([l[2] for l in self.masks[split]["total"]]) for split in self.masks]
        )
        ...
        for split in splits:
            ...
            self.sequences[split]["total"] = torch.LongTensor(
                num_elements, self.max_e1 + self.max_e2 + self.max_r
            ).fill_(0)
            ...
            for i, seq in enumerate(sequences[split]):
                ...
                self.sequences[split]["total"][i, start_r:end_r] = torch.LongTensor(
                    seq[1]
                )
                start_e2 = self.max_e1 + self.max_r
                end_e2 = self.max_e1 + self.max_r + len(seq[2])
                self.sequences[split]["total"][i, start_e2:end_e2] = torch.LongTensor(
                    seq[2]
                )

            if split in ["test", "dev"]:
                print(split)
                self.sequences[split]["negative"] = self.sequences[split][
                    "total"
                ].index_select(
                    0,
                    torch.LongTensor(
                        [i for i, j in enumerate(self.data[split]["total"]) if not j[3]]
                    ),
                )
                # self.data[split]['total'][:self.sequences[split]["total"].size(0)]) if not j[3]]))
                self.sequences[split]["positive"] = self.sequences[split][
                    "total"
                ].index_select(
                    0,
                    torch.LongTensor(
                        [i for i, j in enumerate(self.data[split]["total"]) if j[3]]
                    ),
                )
    ...
    def sample_batch(self, split, bs, cat="total", idxs=None):
        ...
        if idxs:
            seqs = self.sequences[split][cat].index_select(
                0, torch.LongTensor(idxs).to(self.sequences[split][cat].device)
            )
        else:
...
def get_generation_sequences(data, split, text_encoder, test, max_e1=10, max_e2=15):
    sequences = []
    count = 0
    ...
    for event1, relation, event2, _ in tqdm(data[split]["total"]):
        ...
        if split == "train" and len(e1) > max_e1 or len(e2) > max_e2:
            discarded.append(count)
            count += 1
            continue

        final = compile_final_sequence(e1, e2, r, text_encoder)
