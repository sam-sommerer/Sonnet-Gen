...
def save_step(model, vocab, optimizer, opt, length, lrs):
    if cfg.test_save:
        name = "{}.pickle".format(
            utils.make_name(opt, prefix="garbage/models/", is_dir=False, eval_=True)
        )
    else:
        name = "{}.pickle".format(
...
def save_eval_file(opt, stats, eval_type="losses", split="dev", ext="pickle"):
    if cfg.test_save:
        name = "{}/{}.{}".format(
            utils.make_name(
                opt, prefix="garbage/{}/".format(eval_type), is_dir=True, eval_=True
            ),
            split,
            ext,
        )
        ...
        name = "{}/{}.{}".format(
            utils.make_name(
                opt, prefix="results/{}/".format(eval_type), is_dir=True, eval_=True
            ),
            split,
            ext,
        )
...
def set_max_sizes(data_loader, force_split=None):
    ...
    if force_split is not None:
        data_loader.total_size[force_split] = data_loader.sequences[force_split][
            "total"
        ].size(0)
        return
