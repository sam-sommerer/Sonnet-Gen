...
def batch_conceptnet_generate(
    opt, nums, losses, batch_variables, eval_mode=False, tracking_mode=False
):
    data_loader = batch_variables["data"]
    model = batch_variables["model"]
    split = batch_variables["split"]
...
def mle_steps(
    key, model, input_, targets, attention_mask, loss_reduction="mean", i=None
):
    word_acts = decode(model, input_.unsqueeze(1), attention_mask, i)

    word_dist = train_utils.modify_output_for_loss_fn("nll", word_acts, dim=-1)

    # Compute losses
    loss = F.nll_loss(
        word_dist.view(-1, word_dist.size(-1)), targets, reduction=loss_reduction
    )
...
def update_generation_losses(losses, nums, micro, macro, bs, length, loss, split):
    if split == "train":
