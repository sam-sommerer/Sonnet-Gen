...
b = [
    tuple(j)
    for j in data_loader.sequences[split]["positive"][
...
eval_file_name = args.model_name.replace(
    "sample_greedy", "sample_{}".format(args.decoding_strategy)
)
    ...
    for idx in tqdm(total):
        ...
        init = "".join(
            [
                ...
                for i in XMB[:, : data_loader.max_e1].squeeze().tolist()
                if i
            ]
        )
        ...
        attr = "".join(
            [
                text_encoder.decoder[i].replace("</w>", " ")
                for i in XMB[:, start:end].squeeze(0).tolist()
                if i
            ]
        ).strip()
        ...
        for _ in range(args.gen_len):
            ...
            if (
                beam_toks == end_token
            ).sum().item() == args.beam or _ == context_size_e2 - 1:
                break
