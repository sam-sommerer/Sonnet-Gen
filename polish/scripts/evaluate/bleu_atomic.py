...
categories = data_params[
    "categories"
]  # sorted(["oReact", "oEffect", "oWant", "xAttr", "xEffect", "xIntent", "xNeed", "xReact", "xWant"])
...
for i in range(data_loader.sequences[split]["total"].size(0)):
    ...
    init = "".join(
        [
            data_loader.vocab_decoder[i].replace("</w>", " ").replace("<blank>", "___ ")
            for i in tmp[:-1].squeeze().tolist()
            if i
        ]
    )
    ...
    ref = "".join(
        [
            data_loader.vocab_decoder[i].replace("</w>", " ").replace("<blank>", "___ ")
            for i in Ref.squeeze().tolist()
            if i and i != data_loader.vocab_encoder["<END>"]
        ]
    )
...
for category in categories:
    ...
    for gen in tqdm(temp_gens):
        ...
        clean_list_of_refs = [
            [j for j in i.split() if j != "<unk>" and j != "<END>"]
            for i in list_of_refs
        ]
        clean_list_of_gens = [
            [j for j in i.split() if j != "<unk>" and j != "<END>"]
            for i in list_of_gens
        ]

        if (
            sum([i == ["none"] for i in clean_list_of_refs]) / len(clean_list_of_refs)
            > 1 / 3
        ):
            continue
...
write_obj = {"total": total, "category": {}}
write_obj["category"] = {
    "raw": total_bl,
    "counts": total_count,
    "bleu": {
        category: total_bl[category] / total_count[category] for category in total_bl
    },
}
