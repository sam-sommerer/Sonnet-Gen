...
training_gens = [
    i.split("\t")[:3] for i in open(args.training_set_file, "r").read().split("\n") if i
]
...
print(
    "% new o:   {}".format(
        len([i for i in evaluation_e2s if i not in train_e2s]) / len(evaluation_rels)
    )
)
print(
    "% new sro: {}".format(
        len([i for i in evaluation_rels if i not in train_rels]) / len(evaluation_rels)
    )
)
